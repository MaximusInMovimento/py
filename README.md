...
### _Выводы:_
_В данном коде длина предложения определяется командой len, метод lower позваляет преобразовать предложение к нижнему регистру, методы count позволяют посчитать количество вхождений гласных букв, метод replace происводит замену в тексте, а с помощью использования регулярных выражений устанавливается, начинается ли предложение на The, и заканчивается ли на end_
